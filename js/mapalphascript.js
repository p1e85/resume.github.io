...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Element References ---
    // All element lookups are now safely inside this block
    ...
    const validateSignUpForm = () => {
        if (!emailInput || !passwordInput || !usernameInput || !ageCheckbox || !authActionBtn) return;
...
function initializeMapLayers() {
    ...
    if (!map.getLayer('user-pins-icons')) {
        map.addLayer({ id: 'user-pins-icons', type: 'symbol', source: 'user-pins-source', minzoom: 14, layout: { 'icon-image': 'camera-15', 'icon-size': 1.5, 'icon-allow-overlap': true } });
    }
    ...
    if (!map.getLayer('community-pins-icons')) {
        map.addLayer({ id: 'community-pins-icons', type: 'symbol', source: 'community-pins-source', minzoom: 14, layout: { 'icon-image': 'marker-15', 'icon-size': 1.5, 'icon-allow-overlap': true } });
    }
}
